...
class Flute1:
    ...
    def mos(self, datafile, data_dict):
       ...
       try:
           v_fb1, v_fb2, t_ox, n_ox, Q_ox, c_acc = pqc.analyse_mos_data(datafile)
        
       except Exception as err:
           print(str(err))
           v_fb2, c_acc, n_ox = float('NaN'), float('NaN'), float('NaN')
    ...
    def fet(self, datafile, data_dict):
       ...
       try:
           v_th = pqc.analyse_fet_data(datafile)
       except:
           v_th = float('NaN') 
    ...
    def vdp(self, datafile, data_dict, linewidth=False):
       ...
       try:
           r_sheet, lbl = pqc.analyse_van_der_pauw_data(datafile)
    ...
    def capacitor(self, datafile, data_dict):
       ...
       try:  
           c_mean, c_median = pqc.analyse_capacitor_data(datafile)
...
class Flute2:
    ...
    def gcd(self, datafile, data_dict):
        ...
        par_dict = ({'I_surf [pA]': round(i_surf,2) if not math.isnan(i_surf) else i_surf,
              ...
              'Vth_acc [V]': round(vfb_acc,2) +5 if not math.isnan(vfb_acc) else vfb_acc,
