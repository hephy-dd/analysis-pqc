...
\begin{center}
    ...
    \begin{tabular}{ |l|{% for col in flute %}R{0.6cm}|{% endfor %} }
        ...
        {% for i in range(0, dataseries['vdp_pstop_r']|length ) %}{{ startRowStr }}{{ labelformat.format(dataseries['xlabels'][i]) }}{% for key in flute %}{{ valueformat.format(dataseries[key].getValueString(i), statusdict[dataseries[key].getStatus(i)]) }}{% endfor %}{{ endRowStr }}{% endfor %}
        \hline
        {% for i in range(dataseries['vdp_pstop_r']|length, dataseries['vdp_pstop_r']|length+5) %}{{ startRowStr }}{{ labelformat.format(dataseries['xlabels'][i]) }}{% for key in flute %}{{ valueformat.format(dataseries[key].getValueString(i), statusdict[dataseries[key].getStatus(i)]) }}{% endfor %}{{ endRowStr }}{% endfor %}
